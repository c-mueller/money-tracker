...
func (_q *CategoryQuery) QueryHousehold() *HouseholdQuery {
...
func (_q *CategoryQuery) QueryRecurringExpenses() *RecurringExpenseQuery {
...
func (_q *CategoryQuery) WithHousehold(opts ...func(*HouseholdQuery)) *CategoryQuery {
	query := (&HouseholdClient{config: _q.config}).Query()
...
func (_q *CategoryQuery) WithRecurringExpenses(opts ...func(*RecurringExpenseQuery)) *CategoryQuery {
	query := (&RecurringExpenseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := _q.withHousehold; query != nil {
		if err := _q.loadHousehold(ctx, query, nodes, nil,
			func(n *Category, e *Household) { n.Edges.Household = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *CategoryQuery) loadHousehold(ctx context.Context, query *HouseholdQuery, nodes []*Category, init func(*Category), assign func(*Category, *Household)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Category)
	for i := range nodes {
		if nodes[i].household_categories == nil {
			continue
		}
		fk := *nodes[i].household_categories
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "household_categories" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *CategoryQuery) loadRecurringExpenses(ctx context.Context, query *RecurringExpenseQuery, nodes []*Category, init func(*Category), assign func(*Category, *RecurringExpense)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.category_recurring_expenses
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_recurring_expenses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_recurring_expenses" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
