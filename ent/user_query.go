...
func (_q *UserQuery) loadHouseholds(ctx context.Context, query *HouseholdQuery, nodes []*User, init func(*User), assign func(*User, *Household)) error {
	...
	query.Where(predicate.Household(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.HouseholdsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_households
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_households" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_households" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
